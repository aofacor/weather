...
let html = '<div class="col">'
  html = html + '<h3>' + icon(today.icon) + ' </h3>';
  html = html + '<h4>' + today.temperature + '</h4>';
  html = html + '<h5>' + today.summary + '</h5>';
  html = html + '</div></div>';
$(".current").append(html);
$(".current").fadeIn(500);
...
for (let i=1; i<7; i++){
let getforecast = response.daily.data[i];
console.log(getforecast.apparentTemperatureHigh);
console.log(getforecast.summary);
console.log(getforecast.icon);


let htmlp = '<div class="col">'
  htmlp = htmlp + '<h3>' + icon(getforecast.icon) + ' </h3>';
  htmlp = htmlp + '<h4>' + Math.round(getforecast.temperatureHigh) +' | '+ Math.round(getforecast.temperatureLow) + '</h4>';
  htmlp = htmlp + '<h5>' + getforecast.summary + '</h5>';
  htmlp = htmlp + '</div></div></div>';
$(".forecast").append(htmlp);
